...
NEURONS_FC_2 = 1024
DROPOUT_FC12 = 0.5
FN_OPTIMIZER = optimizers.SGD(learning_rate=0.0001, momentum=0.5)
...
tensorflow.random.set_seed(PROCESS_SEED)
...
def callbacks():
    ...
    cb_cpt_best = ModelCheckpoint(filepath=cpt_best, monitor='val_accuracy', save_best_only=True, save_weights_only=True, verbose=1)
    cb_cpt_last = ModelCheckpoint(filepath=cpt_last, monitor='val_accuracy', save_best_only=False, save_weights_only=True, verbose=0)
    cb_tbd = TensorBoard(log_dir=tbd_dir)
...
def plot(train_history):
    ...
    if 'accuracy' in train_history.history:
        train_acc = train_history.history['accuracy']
        val_acc = train_history.history['val_accuracy']
        acc_label = 'accuracy'
        val_acc_label = 'val_accuracy'
    else:
        train_acc = train_history.history['acc']
        val_acc = train_history.history['val_acc']
        acc_label = 'acc'
        val_acc_label = 'val_acc'
    pyplot.plot(train_acc, label=acc_label)
    pyplot.plot(val_acc, label=val_acc_label)
...
def train():
    ...
    # train model
    train_history = model.fit(data_flow_train,
